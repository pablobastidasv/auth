...
@NamedQueries({
    @NamedQuery(name = FIND_BY_USERNAME_AND_TENANT,
        query = "SELECT u FROM User u JOIN u.tenants t LEFT JOIN u.roles g WHERE u.username = :" + USERNAME_FIELD + " AND t.id = :" + TENANT_FIELD)
})
@Table(name = "users")
public class User {
    ...
    @JoinTable(name = "user_x_tenant",
        ...
        uniqueConstraints = @UniqueConstraint(name = "unq_user_tenant", columnNames = {"user_id", "tenant_id"})
    )
    ...
    @JoinTable(name = "user_x_role",
        ...
        uniqueConstraints = @UniqueConstraint(name = "unq_user_role", columnNames = {"user_id", "role_id"})
    )
